...
def solve():
    np, ns, k, H = GM() #number of passengers, number of steps, height of escalator step, height of vlad
    heights = GM() #height of each passenger
    # diff steps, height diff is same as height diff of steps
    count = 0
    for height in heights: #height of each passenger
        heightdifference = heightdiff(height, H) #diff between vlad and passenger height
        if (heightdifference % k == 0 and 1 <= heightdifference//k < ns):
            count+=1
        '''    
        for step in range(1, ns):
            escalatorheight = k*step
            if heightdiff(heightdifference,escalatorheight) == 0:
                count+=1
                break
            else:
                escalatorheight-=ns
