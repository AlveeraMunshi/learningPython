...
def solve():
    n, d, h = GM() #number of branches, length of base, height of branches
    branches = GL()
    area = [0 for _ in range(n)]
    for i in range(n):
        b = branches[i] #current branch
        if branches[i+1] - branches[i] >= h:
            area[i] = (d*h)/2
        else:
            ogside = sqrt((b/2)**2 + h**2)
            newbase = 
            area[i] = (b+newbase)/2 * (branches[i+1] - branches[i])
