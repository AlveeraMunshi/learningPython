...
def solve():
    n, k = GM() #number of days. max diff between expected and actual air temp
    a = GL() # exp
    b = GL() # act
    sorteda = []
    for i in range(n):
        sorteda.append((a[i], i))
    sorteda.sort()
    sortedb = []
    for i in range(n):
    ...
    matched = []
    for i in range(n):
        matched.append((sorteda[i][1], sortedb[i][0]))
    matched.sort()
    ans = "".join(str(matched[i][1]) + " " for i in range(n))
